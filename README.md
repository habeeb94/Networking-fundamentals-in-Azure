...
###### $FW_Subnet = New-AzVirtualNetworkSubnetConfig -Name AzureFirewallSubnet -AddressPrefix 10.0.1.0/26
###### $Workload_Subnet = New-AzVirtualNetworkSubnetConfig -Name Workload-Subnet -AddressPrefix 10.0.2.0/24
...
###### Set-AzVirtualNetworkSubnetConfig -VirtualNetwork $Lab_VN -Name Workload-Subnet -AddressPrefix 10.0.2.0/24 -RouteTable $routeTableDG | Set-AzVirtualNetwork
...
###### $AppRule1 = New-AzFirewallApplicationRule -SourceAddress 10.0.2.0/24 -protocol http, https -TargetFqdn www.google.com
...
###### $NetworkRule1 = New-AzFirewallNetworkRule -Name Allow-DNS -SourceAddress 10.0.2.0/24  -DestinationAddress 209.244.0.3, 209.244.0.4 -DestinationPort 53 -protocol UDP

II. Define the Network rule collection (This is where to explicitly define and store the defined Network rules)
###### $NetworkRuleCollection = New-AzFirewallNetworkRuleCollection -Name Network-collect-01 -Priority 200 -ActionType Allow -Rule $NetworkRule1

III. Update the Azure Firewall that was previously deployed with the newly created Network rule 
###### $AzFW.NetworkRuleCollection.Add($NetworkRuleCollection)
###### Set-AzFirewall -AzureFirewall $AzFW


IV Configure the DNS Server of the Network Interface Card (NIC) to the Destination address used in the Network Rule
...
II. We run a lookup of the two uri (google.com and facebook.com) to see if it will pass through the DNS Server in the Bastion VM PowerShell window (This will test if the VM can reach a DNS server and resolve the domain name)
###### ns lookup www.google.com
###### ns lookup www.facebook.com

III. Then we invoked the web-request of both google.com and facebook.com (Testing http and https connectivity) . It was established that google.com succeeded but facebook.com failed as it is only google.com that was allowed as the target FQDN in the Application Rule.
###### Invoke-WebRequest -uri https://www.google.com
###### Invoke-WebRequest -uri https://www.facebook.com

###### Cleanup Resources
I. Define the cleaning process
###### $Clean_R = Remove-AzResourceGroup -Name Lab-Resources -Force -AsJob 

II. To ensure that the PowerShell Window does not stop while deleting the Resources.
###### Wait-Job -Id $Clean_R.Id
